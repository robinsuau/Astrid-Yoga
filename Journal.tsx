          ...
          <FadeInSection key={service.id} delay={`${index * 150}ms`}>
            <div className="bg-white p-12 rounded-3xl text-center border border-black/5 hover:-translate-y-2 hover:shadow-xl transition-all duration-500 h-full flex flex-col justify-between">
              <div>
                ...
                {service.capacity && (
                  <p className="text-[#666] text-sm font-bold mb-6 uppercase tracking-wider">
                    {service.capacity.includes('Studio Lemon') ? (
                      <>
                        Sur événement ou au{' '}
                        <a 
                          href="https://studiolemonclfd.com/" 
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="text-[#5A6C57] underline hover:text-[#4A5D4B]"
                        >
                          Studio Lemon
                        </a>
                      </>
